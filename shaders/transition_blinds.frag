...
layout(push_constant) uniform PushConstants {
    float progress;
    float angle;      // 0 = horizontal blinds, 90 = vertical blinds
    float pos_x;
    float pos_y;
    float wave_x;     // number of strips (default 20)
    float wave_y;
...
void main() {
    // Choose axis based on angle: near 0 = horizontal strips, near 90 = vertical
    float rad = radians(pc.angle);
    float axis_blend = abs(sin(rad));
    float coord = mix(v_uv.y, v_uv.x, axis_blend);

    // Number of strips from wave_x parameter
    float num_strips = max(pc.wave_x, 2.0);
