#version 450
// GL Transitions — BookFlip by hong
// License: MIT
...
vec2 skewRight(vec2 p) {
    float skewX = (p.x - pc.progress) / (0.5 - pc.progress) * 0.5;
    float skewY = (p.y - 0.5) / (0.5 + pc.progress * (p.x - 0.5) / 0.5) * 0.5 + 0.5;
    return vec2(skewX, skewY);
}

vec2 skewLeft(vec2 p) {
    float skewX = (p.x - 0.5) / (pc.progress - 0.5) * 0.5 + 0.5;
    float skewY = (p.y - 0.5) / (0.5 + (1.0 - pc.progress) * (0.5 - p.x) / 0.5) * 0.5 + 0.5;
    return vec2(skewX, skewY);
}

vec4 addShade() {
    float shadeVal = max(0.7, abs(pc.progress - 0.5) * 2.0);
    return vec4(vec3(shadeVal), 1.0);
}

vec4 transition(vec2 p) {
    float pr = step(1.0 - pc.progress, p.x);
