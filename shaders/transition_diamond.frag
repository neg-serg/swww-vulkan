...
void main() {
    // Diamond reveal using Manhattan distance (L1 norm)
    vec2 center = vec2(pc.pos_x, pc.pos_y);
    float dist = abs(v_uv.x - center.x) + abs(v_uv.y - center.y);

    // Max possible Manhattan distance from center to any corner
    float max_dist = max(center.x, 1.0 - center.x) + max(center.y, 1.0 - center.y);
