#version 450
// GL Transitions — CircleCrop by fkuteken
// License: MIT
...
vec4 transition(vec2 p) {
    vec2 ratio2 = vec2(1.0, 1.0 / pc.screen_aspect);
    float s = pow(2.0 * abs(pc.progress - 0.5), 3.0);

    float dist = length((vec2(p) - 0.5) * ratio2);
    return mix(
        pc.progress < 0.5 ? getFromColor(p) : getToColor(p),
        bgcolor,
        step(s, dist)
    );
}
