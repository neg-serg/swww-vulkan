...
// Hash function for pseudo-random noise from UV coordinates
float hash(vec2 p) {
...
void main() {
    // Generate noise value [0,1] per pixel
    // Multiply by large number for pixel-level granularity
    float noise = hash(v_uv * 1000.0);

    // Pixel dissolves when noise < progress (with soft edge)
    float edge = 0.05;
    float mask = smoothstep(pc.progress - edge, pc.progress + edge, noise);
    ...
    // mask=1 means noise > progress → show old; mask=0 means dissolved → show new
